...
- Conversation list: polls every 20 seconds while the user conversation list is focused.
- Admin conversation list: polls every 30 seconds while the admin conversation list is focused.
- Alerts and unread alert counts: poll every 12 seconds while enabled.
- Activity summary: optional lightweight polling every 45 seconds while enabled.
...
- Employer jobs, contractor job browse, job details, my offers, contact unlock, contacts, contact details, reviews, and wallet refetch on screen focus.
...
- Offers: `['offers']`, `['offers', 'job', jobId]`, `['offers', 'mine']`
...
- Create, update, withdraw, or select offer: cache the changed offer, invalidate job details, job offers, my offers, activity, contacts, messages, and notifications.
...
- Contractor sees selected offers and activity updates on focus or within the activity poll interval.
